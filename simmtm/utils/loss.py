...
class ContrastiveLoss(nn.Module):
    ...
    def forward(self, batch_emb_om, batch_x):
        ...
        y_true = torch.cat((torch.ones(cur_batch_shape[0], positives.shape[-1]) / positives.shape[-1],
                            torch.zeros(cur_batch_shape[0], negatives.shape[-1])), dim=-1).to(self.device).float()
...
class RebuildLoss(torch.nn.Module):
    ...
    def forward(self, similarity_matrix, batch_emb_om, batch_emb_o, batch_x):
        ...
        rebuild_oral_batch_emb = rebuild_batch_emb[:oral_batch_shape[0]].reshape(oral_batch_shape[0],
                                                                                 cur_batch_shape[1], -1)
