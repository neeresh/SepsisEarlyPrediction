...
def train_model(model, train_loader: DataLoader, test_loader: DataLoader, epochs: int, class_0_weight=None,
    ...
    # Saving the model
    save_model(model, model_name=f"./saved_models/gtn/baseline_gtn{config['num_epochs']}.pkl")
...
if __name__ == '__main__':
    ...
    pretrained_d_input, pretrained_d_channel, pretrained_d_output = (336, 191, 2)
    model = Transformer(d_model=config['d_model'], d_input=pretrained_d_input, d_channel=pretrained_d_channel,
                        d_output=pretrained_d_output, d_hidden=config['d_hidden'], q=config['q'],
                        v=config['v'], h=config['h'], N=config['N'], dropout=config['dropout'],
                        pe=config['pe'], mask=config['mask'], device=device).to(device)

    ####################################################################################################################
    # # Creating a model instance with pre-trained settings
    # pretrained_d_input, pretrained_d_channel, pretrained_d_output = (198, 6, 2)
    # model = Transformer(d_model=config['d_model'], d_input=pretrained_d_input, d_channel=pretrained_d_channel,
    #                     d_output=pretrained_d_output, d_hidden=config['d_hidden'], q=config['q'],
    #                     v=config['v'], h=config['h'], N=config['N'], dropout=config['dropout'],
    #                     pe=config['pe'], mask=config['mask'], device=device).to(device)
    ...
    # model.load_state_dict(torch.load(os.path.join(project_root(), 'data', 'pretrain_datasets',
    #                                               'saved_models', 'wafer_95.pkl')))
    #
    # # # Freezing all layers (except input); output layer has 2 classes.
    ...
    #
    # # Replacing embedding input (input layer) with new layer and trianing it
    # model.embedding_channel = nn.Linear(d_channel, model.embedding_channel.out_features).to(device)
    # model.embedding_input = nn.Linear(d_input, model.embedding_input.out_features).to(device)
    # model.gate = torch.nn.Linear(pretrain_params['d_model'] * d_input + pretrain_params['d_model'] * d_channel,
    #                             2).to(device)
    # model.output_linear = torch.nn.Linear(pretrain_params['d_model'] * 336 + pretrain_params['d_model'] * 191,
    #                                       d_output).to(device)
    #
    # model._d_input = d_input
    #
    # # Total number of trainable layers
    # counter = 0
    # for name, param in model.named_parameters():
    #     if param.requires_grad:
    #         counter += 1
    # print(f"Total number of trainable layers: {counter}")
    ####################################################################################################################

    # model = nn.DataParallel(model)
    metrics = train_model(model, train_loader, val_loader, epochs=30)
