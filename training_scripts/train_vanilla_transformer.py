...
from models.vanilla.model.transformer import Transformer
from utils.config import vanilla_param
from utils.loader import make_loader
from utils.path_utils import project_root
from utils.plot_metrics import plot_losses_and_accuracies

device = 'cuda'
config = vanilla_param
...
def train_model(model, train_loader: DataLoader, test_loader: DataLoader, epochs: int, class_0_weight=None,
    ...
    # Saving the model
    save_model(model, model_name=f"./saved_models/vanilla/vanilla_transformer_{config['num_epochs']}.pkl")
...
if __name__ == '__main__':
    ...
    model = Transformer(device=device, d_model=191, max_len=336).to(device)
