...
def get_pretrain_finetune_test_datasets():
    ...
    finetune_sepsis_path = os.path.join(project_root(), 'data', 'tl_datasets', 'finetune', 'is_sepsis.txt')

    with open(finetune_lengths_path, 'w') as f:
        [f.write(f'{length}\n') for length in finetune_lengths.squeeze()]

    with open(finetune_sepsis_path, 'w') as f:
        [f.write(f'{sepsis}\n') for sepsis in finetune_sepsis.squeeze()]

    # Saving fine-tuning as .pt
    destination_path = os.path.join(project_root(), 'data', 'tl_datasets', 'finetune')
    torch.save(finetune, destination_path + '/finetune.pt')
